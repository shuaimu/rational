...
/**
 * The browser's own `<select>`, dressed. Forms use it: it submits with the
 * form, works with `selectOption` in a test, and needs no portal. The Radix
 * `Select` below is for pickers inside menus and toolbars.
 */
export function NativeSelect({
  ...
  size?: "default" | "sm";
  /** Classes for the wrapper, to size an inline select (`w-auto`, `min-w-40`). */
  wrapperClassName?: string;
}) {
  return (
    <span data-slot="native-select" className={cn("relative inline-flex w-full", wrapperClassName)}>
      <select
        data-slot="native-select-control"
        data-size={size}
        multiple={multiple}
        className={cn(
          "w-full appearance-none rounded-md border border-input bg-transparent py-1 pl-3 text-base shadow-xs outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 md:text-sm",
          multiple ? "min-h-24 pr-3" : size === "default" ? "h-9 pr-8" : "h-8 pr-8 text-sm",
          className,
        )}
        {...props}
      >
